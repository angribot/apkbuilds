...
pushes each eligible package origin before the next. The ordered updater
manifest under `packages/updaters` registers every package origin with its
updater and updater test, explicitly marks package origins without update
automation as `-|-`, and is validated before any updater runs. This preserves
successful package origin
